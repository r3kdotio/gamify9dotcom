...
public class RunGameAndScoreServiceTest {

  @Test
  public void shouldRespondWithSuccessForSinglePointAndGameStartedIndicators() {
    //given
    String givenString = "Java version is: 9\n"
            ...
            + "Software renderer disposed\n"
            + "Scored point. Player now has 1 point";
    //when
    RunGameAndScoreService underTest = new RunGameAndScoreService();
  ...
  public void shouldRespondWithErrorIfNoPointIsScored() {
    ...
    ResponseEntity<ChallengeReponse> response = underTest.readInput(givenString);
    //then
    assertEquals("The game has started, but you have not scored a point", response.getBody().getMessage());
    assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode());
  }
