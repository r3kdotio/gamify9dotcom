...
export class CompileWithModuleDirComponent extends BaseChallengeComponent {
  ...
  submit(input){
    ...
    this.challenge1Service.compileWithModuleDir(input).subscribe(res => this.evaluateResponse(res), error => { this.error(error) });
  }
  next() {
