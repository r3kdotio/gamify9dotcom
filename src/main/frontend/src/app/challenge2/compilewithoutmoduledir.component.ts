...
export class CompileWithoutModuleDirComponent extends BaseChallengeComponent{
  ...
  submit(input){
    ...
    this.challenge1Service.compileWithoutModuleDir(input).subscribe(res => this.evaluateResponse(res), error => { this.error(error) });
  }
  next() {
