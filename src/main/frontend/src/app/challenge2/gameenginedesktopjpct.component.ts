...
<div class="animated fadeIn">
        ...
        Let's add the automatic module "requires jpct;" to the module-info.java.<br/>
        This should now compile with a single warning.<br/>
            </div>
            <div class="card-body">
...
export class GameEngineDesktopJPCTComponent extends BaseChallengeComponent {
  ...
  submit(input) {
    ...
    this.challenge1Service.readInputWithModuleInfoDesktopAndJPCT(input).subscribe(res => this.evaluateResponse(res), error => { this.error(error) });
  }
  next() {
    this.router.navigate(['./challenge2/5']);
  }
}